...
from tests.testlib import TestBase, SupportedBrokers, QPID, OPENAMQ
...
class TxTests(TestBase):
    ...
    def test_commit(self):
        ...
        for i in range(1, 5):
            ...
            self.assertEqual(b"TxMessage %d" % i, msg.content.body)

        msg = yield queue_b.get(timeout=1)
        self.assertEqual(b"TxMessage 6", msg.content.body)

        msg = yield queue_a.get(timeout=1)
        self.assertEqual(b"TxMessage 7", msg.content.body)
