...
def _get_host():
    return os.environ.get('TXAMQP_TEST_HOST', 'localhost')

def _get_port():
    return int(os.environ.get('TXAMQP_TEST_PORT', 5672))
...
class TestBase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.broker == RABBITMQ:
            self.spec = '../txamqp/specs/standard/amqp0-9.stripped.xml'
        elif self.broker == OPENAMQ:
            self.spec = '../txamqp/specs/standard/amqp0-9.stripped.xml'
        elif self.broker == QPID:
            self.spec = '../txamqp/specs/qpid/amqp.0-8.xml'
        else:
            raise RuntimeError(
